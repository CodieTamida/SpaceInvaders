...
void CreateOpponents() {
  ...
  ASSERT_NE(opponents[0], nullptr) << "The function should create an Opponent object in the heap.";
}
...
void LaunchProjectiles() {
  ...
  for (int i = 0; i < 100 && !has_projectile; i++) {
    ...
    std::vector<std::unique_ptr<OpponentProjectile>>& projectiles = my_game.GetOpponentProjectiles();
    if (my_game.GetOpponentProjectiles().size() > 0 && projectiles[0] != nullptr) {
      has_projectile = true;
    }
  }
  ASSERT_TRUE(has_projectile) << "Opponents should launch a projectile and store the projectile in the heap.";
}
...
void OnAnimationStep() {
  ...
  std::cout << "number of opponents is " << opponents.size();
  //std::cout << " number of projectiles is " << opponent_projectile.size();
...
void OnMouseEventPressed() {
  ...
  ASSERT_NE(player_projectiles[0], nullptr) << "The player projectile should be created on the heap.";
}
